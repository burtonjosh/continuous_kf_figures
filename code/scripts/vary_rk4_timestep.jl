...
if isfile(string(saving_path_data,"rk4_timestep_multiple_diags_ll.csv"))
    ll_values = readdlm(string(saving_path_data,"rk4_timestep_multiple_diags_ll.csv"))
else
    ll_values = zeros(
    ...
    for (i, euler_timestep) in enumerate(euler_timesteps)
        for (j, diagonal_step) in enumerate(off_diagonal_steps)
            ll_values[i, j] = calculate_log_likelihood_at_parameter_point(
                protein_observations,
                p,
                measurement_std^2;
                alg=RK4(),
                off_diagonal_steps=diagonal_step,
                euler_dt = euler_timestep
            )
        end
    end
    writedlm(string(saving_path_data,"rk4_timestep_multiple_diags_ll.csv"),ll_values)
end
...
plot!(
    xlabel="4-th order Runge Kutta timestep (minutes)",
    ylabel="log-likelihood"
)

savefig(string(saving_path_figure, "rk4_timestep_multiple_diags.pdf"))
...
savefig(string(saving_path_figure, "rk4_timestep_error_multiple_diags.pdf"))
...
savefig(string(saving_path_figure, "rk4_timestep_relative_error_multiple_diags.pdf"))


using BenchmarkTools

if isfile(string(saving_path_data,"rk4_timestep_multiple_diags_benchmark.csv"))
    median_benchmark = readdlm(string(saving_path_data,"rk4_timestep_multiple_diags_benchmark.csv"))
else
    ...
    writedlm(string(saving_path_data,"rk4_timestep_multiple_diags_benchmark.csv"),median_benchmark)
end
...
savefig(string(saving_path_figure, "rk4_timestep_benchmark_multiple_diags.pdf"))